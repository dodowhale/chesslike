...
- [x] 서버 랭킹 — Hono `/api/leaderboard` SQLite 연동 구현 완료 (Bun:Sqlite 및 CORS 적용)
- [ ] 사용자 인증 — **M6+**
...
- [x] 아이템 아이콘 30종 (`adventure/items/{itemId}.png` 32×32, 풀과 1:1 매칭) — 자산 도입 및 UI 통합 완료 (AdventureInventory 및 AdventureShop 카드에 개별 아이콘 PNG 적용)
- [x] 막별 배경 3종 (`adventure/backgrounds/act{1,2,3}.png` 480×270 픽셀 아트) — 자산 도입 및 UI 통합 완료 (AdventureMap, AdventureBattle, AdventureBoss 배경 레이어에 적용)
...
- [x] **자산 UI 통합 (완료)** — 노드/보스/캐릭터/아이템/배경 PNG를 실제 화면에서 사용하도록 통합 완료.
...
- [x] 일반 모험 AI를 random에서 약한 Stockfish (Skill Level 1~4)로 교체 완료
...
- [x] `/api/leaderboard` 랭킹 등록·조회 실제 구현
- [x] `/api/achievements/verify` 서버 검증 (조작 방지)
- [ ] 사용자 인증 (옵션, 닉네임 기반)
