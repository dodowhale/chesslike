...
interface Modifier {
  ...
  healPerTurn?: number;
  thornsDamage?: number;
  bindOnHit?: number;        // 피격/스킬 타격 시 대상 속박 턴 수
  weakenOnHit?: number;      // 피격/스킬 타격 시 대상 약화 턴 수
  lifestealRatio?: number;   // 캡처 성공 시 흡혈 비율
}
...
- **인벤토리 단축키**: 모험 모드의 맵 화면과 인벤토리 화면에서 키보드 `i` / `I` 또는 `Escape` 키를 감지하여 맵 ↔ 인벤토리 간의 즉각적인 라우팅 전환(단축키)을 수행한다.
...
- 보스의 HP가 0이 되어도 페이즈가 끝나지 않는다 (스탯/이동 범위가 디버프 처리될 뿐이다). **적 킹(보스)은 일반 공격, 액티브 스킬 데미지, 혹은 매 턴 시작 시의 맹독(Poison) 피해 등으로 인해 HP가 0 이하가 될 때 보드에서 제거되지 않고 HP 0으로 고정(clamp)되어 생존을 유지한다.**
- 페이즈 종료 및 다음 페이즈 진입 조건은 오직 **체크메이트**뿐이다.
- 보스 킹이 체크메이트되어 페이즈가 전환될 때 보드는 부분 리셋(재배치)된다:
  - 보스 킹이 체크메이트되어 다음 페이즈로 가기 전, UI상으로 페이즈 클리어 상황과 부활 예고 및 리셋 규칙을 알려주는 **시각적 인터스티셜 오버레이**가 렌더링되며, 플레이어가 버튼을 클릭해야 다음 페이즈로 진행된다.
...
- **세트 종류 및 효과**:
  - **철의 기사 (`hasIronKnight`)**: 강철 방패 (`iron-shield`) + [가죽 갑옷 (`leather-armor`) / 아이기스 판금 (`aegis-plate`) / 용비늘 갑옷 (`dragon-scale`)]. 최대 HP +15, 피격 시 반사 데미지(thornsDamage) +4 가산.
  - **비전 현자 (`hasArcaneSage`)**: 오크 지팡이 (`oak-staff`) + 룬각인 건틀릿 (`runic-gauntlet`). 기물의 액티브 스킬 사용 시 쿨다운 차감 보너스 1턴 부여 (실제 스킬 사용 시 쿨다운이 `cooldownTurns - 1`로 시작), 스킬 효과 배율 1.3배 상승.
  - **독사의 이빨 (`hasViper`)**: 날카로운 칼 (`sharp-blade`) + 독사의 송곳니 (`serpent-fang`). 보스 킹 일반 공격 혹은 나이트/퀸/킹의 액티브 스킬 타격 성공 시, 대상에게 3턴간 3 데미지를 입히는 '맹독' 디버프 부여.
  - **태양의 가호 (`hasSunShield`)**: 튼튼한 망토 (`sturdy-cloak`) + 햇빛으로 벼린 검 (`sunforged-blade`). 기물 공격력 +2 증가, 아군 기물이 적 캡처 성공 시 아군 킹의 HP +8 회복 및 장착 기물의 기본 공격력 +2 영구 누적.
