...
### Fixed
- 암살자단 패시브 설명이 이전 기획안("점프 시 추가 데미지")으로 표시되던 오류를 실제 동작("적 기물 캡처 성공 시 공격력 +1 영구 누적")에 맞춰 수정 및 다국어 정렬.
- `TODO.md`에 M6+에서 이미 구현 완료된 외부 정식 도트 자산 도입 및 Phaser Tween/Sprite identity 리팩토링 항목을 완료 상태로 동기화.

## [0.3.0] — 2026-07-11

### Added
- 모험 모드 상점 새로고침(Reroll) 기능 추가 (15골드 소모하여 판매 아이템 3개 재추첨 및 품절 상태 초기화).
- 암살자단(Assassins) 전용 패시브 `assassins-jump` (나이트가 캡처 시 기본 공격력 +1 영구 누적) 실구현 완료.
- 모험 전투 중 실시간 상황(이동, 캡처, 데미지, 반격 소멸, 킹 전가 피해, 액티브 스킬 사용 등)을 모니터링할 수 있는 **전투 행동 로그 패널(Combat Log Panel)** 구현 완료.
- 모험 모드 해금 캐릭터들(암살자단, 신성단, 요새단, 혼돈단)에게 시작 인벤토리 아이템을 다변화하여 차등 지급 적용.

### Changed
- 신성단(Saints) 패시브 `saints-blessing` (매 턴 킹 HP +1) 정산 수정: 아군 기물 전체 힐로 적용되던 버그를 수정하여 백의 킹 기물에만 턴 시작 힐이 부여되도록 타겟 조율.
- 상태이상 부여 아이템 밸런스 조율:
  - `binding-chain` (속박의 사슬): 100% 확정 속박에서 50% 확률형 속박(1턴)으로 조정.
  - `weaking-totem` (약화의 토템): 100% 확정 2턴 약화에서 50% 확률형 1턴 약화로 조정.
- 흡혈 아이템 밸런스 조율:
  - `vampire-fang` (뱀파이어의 송곳니): 캡처 시 흡혈 비율을 50%에서 30%로 하향 조정하고, 1회당 최대 힐량을 15 HP 한도로 제한.
- 아이템 스탯 조정:
  - `knight-spurs` (기사의 박차): 스탯 인플레이션을 방지하기 위해 제공 스탯을 { hp:10, attack:3, thornsDamage:2 }로 하향 튜닝.
- 몬스터 & 보스 AI Elo 동적 조정: Act 1 보스(Elo 1200->1100, Skill Level 3->2) 및 Act 2 보스(Elo 1500->1400, Skill Level 5->4) 난이도를 미세 하향 조율하여 자연스러운 입문 장벽 형성.


## [0.2.0] — 2026-07-11

### Added
- 모험 모드 신규 상태이상 시스템(속박 `Bind`, 약화 `Weaken`) 실구현 및 기동 제한/공격력 50% 반감 적용.
- 모험 모드 신규 아이템 3종 (`binding-chain` 속박의 사슬, `weaking-totem` 약화의 토템, `vampire-fang` 뱀파이어의 송곳니) 추가.
- 모험 모드 신규 해금 캐릭터 2종 ('요새단', '혼돈단') 실구현 및 해금 비용/시작 진형 연동.
- 캐릭터별 시작 기물 진형(로드아웃) 변경 사항(암살자단: 4나이트, 신성단: 4비숍, 요새단: 4룩) 정밀 동기화.
- 상태이상 및 흡혈/캐슬링 패시브 동작을 완벽히 정합 검증하는 6개의 단위/통합 테스트 케이스 추가.

### Changed
- `AdventureChessManager.ts`의 `effectiveAttack` 및 `legalDestinations`에서 약화 및 속박 디버프 처리를 연동하여 턴 기반 행동이 제약받도록 수정.
- 기물 고유 액티브 스킬(나이트 '번개 돌진', 퀸 '진공의 손길', 킹 '왕의 진노') 타격 시에도 장착 중인 아이템의 상태이상 효과가 타격 대상에게 전파 및 부여되도록 확장.
- 턴 정산 로직 개선: 상태이상 지속 턴 수 차감을 턴 시작 힐 정산(`applyTurnStartHeal`)에서 제거하고, 턴이 실제로 교대되어 끝나는 시점(`applyTurnEndStatusReduction`)으로 변경하여 턴제 디버프의 논리 정합성을 보장.

### Fixed
- 캐슬링 발생 시 킹의 relocation과 연계하여 룩의 `piecesBySquare` 좌표 및 `square` 갱신이 누락되어 룩의 포지션 트래킹이 깨지던 고질적 버그 수정.


## [0.1.1] — 2026-07-08

### Fixed
- 클래식 모드 게임 종료 팝업에서 닫기 버튼 클릭 시, 메인 메뉴 화면으로 정상 이동하도록 수정.
...
- 모험 모드 적 킹(보스)을 일반 공격할 때 독사의 이빨 세트 시너지(`sharp-blade` + `serpent-fang`)를 보유한 아군 기물의 공격인 경우, 보스 킹에게 맹독(3턴간 3 데미지) 디버프가 적용되지 않던 누락 현상을 보완하고 이에 대한 단위 테스트(`AdventureChessManager.test.ts`)를 추가함.
- 모험 모드 턴 시작 시 맹독(Poison) 피해 정산 과정에서 적 킹(보스)의 HP가 0 이하로 떨어질 때, 보드에서 기물이 제거(소멸)되어 버리는 잠재적 버그를 수정함. 보스 킹인 경우에는 기물 삭제 대신 HP를 0으로 고정(clamp)하여 생존을 유지하게 하여 정상적으로 체크메이트 유도가 가능하도록 개선하고 이에 대한 단위 테스트를 추가함.
